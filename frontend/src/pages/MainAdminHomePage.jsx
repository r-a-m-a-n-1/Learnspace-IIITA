...
//     ],
//   },
//   // Continue for Semesters 5-8 as needed
// };
...
//       {/* Navbar */}
//       <nav className="bg-black px-4 py-3 flex flex-col md:flex-row justify-between items-center shadow-md">
...
//           <span className="text-lg md:text-xl text-blue-400 mt-2 md:mt-0">
//             Welcome,&nbsp;
...
//   const handleSubmit = async (e) => {
//     e.preventDefault();
//     const { sem, branch, year, materialType, lectureType, customType, paperType, title, subject, file } = data;
//     const showBranch = sem !== "Semester 1";
//     const isPreviousYear = materialType === "Previous Year Paper";
//     const isLectureNotes = materialType === "Lecture Notes and Assignment";
...
//         }
//       };
//       xhr.onerror = () => { throw new Error("Upload failed. Please try again."); };
//       xhr.onload = async () => {
//         if (xhr.status !== 200 && xhr.status !== 201) {
//           const err = JSON.parse(xhr.responseText);
//           throw new Error(err.error?.message || "Cloudinary upload failed.");
...
//     <form
//       onSubmit={handleSubmit}
//       className="bg-gray-800 p-6 sm:p-8 rounded-xl w-full shadow-xl text-white space-y-6"
//     >
...
//           className="w-full px-4 py-2 rounded-lg bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
//         />
//         <label className="w-full flex flex-col items-center px-4 py-2 bg-gray-700 rounded-lg cursor-pointer hover:bg-gray-600 transition text-center">
//           <span className="text-sm text-gray-200">Select File</span>
//           <input
//             type="file"
//             name="file"
//             onChange={handleChange}
...
//                 {showAdmins && (
//                   <div className="mt-2 w-full max-h-52 bg-gray-700 rounded-md shadow-lg overflow-y-auto scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800">
...
function UploadMaterialForm() {
  ...
  useEffect(() => {
    ...
    if (sem === "Semester 1") {
      ...
      setSubjects(semesterSubjects[sem]?.[branch] || []);
    }
    setData((prev) => ({ ...prev, subject: "" }));
  }, [data.sem, data.branch]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const cloudinaryUrl = `https://api.cloudinary.com/v1_1/${cloudName}/raw/upload`;
      const formData = new FormData();
      const uniqueFileName = `${file.name.split(".")[0]}_${Date.now()}.${file.name
        .split(".")
        .pop()}`;
      formData.append("file", file, uniqueFileName);
      formData.append("upload_preset", uploadPreset);
