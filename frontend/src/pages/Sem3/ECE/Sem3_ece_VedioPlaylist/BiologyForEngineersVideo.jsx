...
const BFE = () => {
  // RBFElace these video IDs with your actual YouTube video IDs (one for each of the 50 videos)
  const videoIDs = [
    "O_bpzGe6rGA",
    "zKnA0qLWeU0",
    "uNMBYoghmjE",
  ...
  const videoDescriptions = [
    "Biology for Engineers in one shot || Engineering biology one shot || Modern Vs Traditional Biology",
    "Biology for engineers in one shot || Classification in one shot|| Plant cell vs Animal cell",
    "Genetics in one shot || Biology for engineers b.tech   || Meiosis vs Mitosis || Single gene disorder",
    "Biology for engineers in one shot || Engineering biology in one shot(part-4) || Kreb Cycle",
    ...
    <div className="flex bg-black text-white w-full min-h-screen">
      ...
      <main className={`flex-1 pt-4 p-4 sm:p-8 ${sidebarOpen ? 'pr-64' : ''}`}>
        ...
        <div className="flex items-center justify-center mb-6 sm:mb-8 relative">
          <h1 className="text-3xl sm:text-4xl font-bold">Biology for Engineers - Video Lectures</h1>
          <button
